...
def run():
    ...
    def generate_account_number():
        ...
        while True:
            acc_num = random.randint(1000000000, 9999999999)  # 10-digit number
            if not check_account_exists(acc_num):  # Ensure the account number is unique
                return acc_num
    ...
    def display_accounts():
        """Retrieve all accounts and their details."""
        connection = create_connection()
        accounts_list = []
        if connection:
            cursor = connection.cursor()
    ...
    def create_account_window():
        ...
        create_window.config(bg="#add8e6")  # Light Blue Background

        tk.Label(create_window, text="Create New Account", bg="#add8e6", font=('Arial', 16, 'bold')).grid(row=0, column=0,
                                                                                                          columnspan=2,
                                                                                                          padx=10, pady=10)

        tk.Label(create_window, text="Name:", bg="#add8e6", font=('Arial', 12)).grid(row=1, column=0, padx=10, pady=10)
        tk.Label(create_window, text="Initial Balance:", bg="#add8e6", font=('Arial', 12)).grid(row=2, column=0, padx=10,
                                                                                                pady=10)
        ...
        submit_button = tk.Button(create_window, text="Create", command=submit, bg="#5cb85c", fg="white",
                                  font=('Arial', 12, 'bold'))
    ...
    def delete_account_window():
        """Open the Delete Account window."""
        ...
        delete_window.config(bg="#add8e6")  # Light Blue Background

        tk.Label(delete_window, text="Delete Account", bg="#add8e6", font=('Arial', 16, 'bold')).grid(row=0, column=0,
                                                                                                      columnspan=2, padx=10,
                                                                                                      pady=10)
        tk.Label(delete_window, text="Account Number:", bg="#add8e6", font=('Arial', 12)).grid(row=1, column=0, padx=10,
                                                                                               pady=10)

        account_entry = tk.Entry(delete_window, font=('Arial', 12))
        account_entry.grid(row=1, column=1, padx=10, pady=10)

        submit_button = tk.Button(delete_window, text="Delete", command=submit, bg="#d9534f", fg="white",
                                  font=('Arial', 12, 'bold'))
    ...
    def deposit_window():
        ...
        def submit():
            ...
            try:
                ...
                if new_balance is not None:
                    messagebox.showinfo("Success", f"Deposited {amount}. New balance: {new_balance:.2f}")
                    deposit_win.destroy()
                else:
        ...
        deposit_win.config(bg="#add8e6")  # Light Blue Background

        tk.Label(deposit_win, text="Deposit Money", bg="#add8e6", font=('Arial', 16, 'bold')).grid(row=0, column=0,
                                                                                                   columnspan=2, padx=10,
                                                                                                   pady=10)

        tk.Label(deposit_win, text="Account Number:", bg="#add8e6", font=('Arial', 12)).grid(row=1, column=0, padx=10,
                                                                                             pady=10)
        ...
        submit_button = tk.Button(deposit_win, text="Deposit", command=submit, bg="#5cb85c", fg="white",
                                  font=('Arial', 12, 'bold'))
        submit_button.grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def withdraw_window():
        ...
        def submit():
            ...
            try:
                ...
                if new_balance is not None:
                    messagebox.showinfo("Success", f"Withdrew {amount}. New balance: {new_balance:.2f}")
        ...
        tk.Label(withdraw_win, text="Withdraw Money", bg="#add8e6", font=('Arial', 16, 'bold')).grid(row=0, column=0,
                                                                                                     columnspan=2, padx=10,
                                                                                                     pady=10)

        tk.Label(withdraw_win, text="Account Number:", bg="#add8e6", font=('Arial', 12)).grid(row=1, column=0, padx=10,
                                                                                              pady=10)
        ...
        submit_button = tk.Button(withdraw_win, text="Withdraw", command=submit, bg="#d9534f", fg="white",
                                  font=('Arial', 12, 'bold'))
        submit_button.grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def display_accounts_window():
        """Open the Display Accounts window."""
        accounts = display_accounts()
        display_win = tk.Toplevel()
        display_win.title("Display Accounts")
        display_win.config(bg="#add8e6")  # Light Blue Background
        ...
        close_button = tk.Button(display_win, text="Close", command=display_win.destroy, bg="#5bc0de", fg="white",
                                 font=('Arial', 12, 'bold'))
    ...
    tk.Button(app, text="Create Account", command=create_account_window, bg="#5cb85c", fg="white",
              font=('Arial', 12, 'bold')).pack(pady=10, fill='x', padx=20)
    tk.Button(app, text="Delete Account", command=delete_account_window, bg="#d9534f", fg="white",
              font=('Arial', 12, 'bold')).pack(pady=10, fill='x', padx=20)
    tk.Button(app, text="Deposit", command=deposit_window, bg="#5bc0de", fg="white", font=('Arial', 12, 'bold')).pack(
        pady=10, fill='x', padx=20)
    tk.Button(app, text="Withdraw", command=withdraw_window, bg="#5bc0de", fg="white", font=('Arial', 12, 'bold')).pack(
        pady=10, fill='x', padx=20)
    tk.Button(app, text="Display Accounts", command=display_accounts_window, bg="#5bc0de", fg="white",
              font=('Arial', 12, 'bold')).pack(pady=10, fill='x', padx=20)
